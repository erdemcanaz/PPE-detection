...
class TrackerSupervisor:
    ...
    def update_trackers_with_detections(self, detections:list[dict]=None):
        ...
        for detection in detections:
            detection["px"] = (detection["bbox_coordinates"][0]+detection["bbox_coordinates"][2])/2 #px-center of the box 
            detection["py"] = (detection["bbox_coordinates"][1]+detection["bbox_coordinates"][3])/2 #py-center of the box 
            ...
            for tracker in self.object_trackers:
                ...
                distance_now = tracker.calculate_distance(detection["px"], detection["py"])
        ...
        for detection in detections:
            if detection not in matched_detections:
                ...
                self.object_trackers.append(Tracker(track_id = self.last_id, max_age = self.MAX_AGE, track_current_info = detection))
...
class Tracker:
    ...
    def __init__(self, track_id: int, max_age: int = None, track_current_info: dict = None):        
        ...
        self.current_position = [track_current_info["px"], track_current_info["py"]]
    ...
    def set_position(self, track_current_info:dict=None) -> None:
        print(f"Tracker {self.TRACK_ID} is updated with new position: {track_current_info['px']}, {track_current_info['py']}")
        self.state = 1
        self.age = 0
        track_current_info["tracker_id"] = self.TRACK_ID
